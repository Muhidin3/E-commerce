...
function HeaderProducts() {
    ...
    <Link href={'/'} style={{marginLeft:'20px',color:'#fff',textTransform:'none'}}>Home</Link>
      <div className='inline-block duration-150' 
          style={{marginLeft:'20px',color:'#fff',textTransform:'none'}} 
          onMouseEnter={()=>setHover(true)} onMouseLeave={()=>setHover(false)} onClick={()=>{setHover(!hover);console.log(hover)}} >
      Categories{hover ? <KeyboardDoubleArrowDown/> : <KeyboardArrowDownRounded/>}

      {hover && (
      <Box sx={{}}>
        <List style={{position:'fixed',top:'60px',margin:0,padding:0,borderRadius:'10px',backgroundColor:theme.palette.background.paper,color:theme.palette.text.primary}} className='z-10'>
