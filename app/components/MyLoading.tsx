...
function MyLoading({bool=true}:{bool?:boolean}) {
  const [boool,setBool] = useState(bool) 

    if (boool) {
      return (
        <div onClick={()=>setBool(false)} className='fixed inset-0 z-50 flex items-center justify-center backdrop-blur-xs bg-black/30'>
          <Box>
